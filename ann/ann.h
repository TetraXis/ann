...
namespace ann
{
	...
	struct ann
	{
		...
		void set_biases_to_rand(float min, float max);
	...
	inline void ann<input_size, output_size, hidden_amount, hidden_size>::forward_propagation()
	{
		...
		neurons.output = weights.last_layer * neurons.hidden[hidden_amount - 1];
		for (unsigned int i = 0; i < output_size; i++)
		{
	...
	template<unsigned int input_size, unsigned int output_size, unsigned int hidden_amount, unsigned int hidden_size>
	inline void ann<input_size, output_size, hidden_amount, hidden_size>::forward_propagation(Eigen::Vector<float, input_size> input_values)
	{
#ifdef ANN_DEBUG
	...
	template<unsigned int input_size, unsigned int output_size, unsigned int hidden_amount, unsigned int hidden_size>
	inline void ann<input_size, output_size, hidden_amount, hidden_size>::set_biases_to_rand(float min, float max)
	{
		for (unsigned int i = 0; i < hidden_amount; i++)
		{
			for (unsigned int j = 0; j < hidden_size; j++)
			{
				biases.hidden[i][j] = rand_float(min, max);
			}
		}

		for (unsigned int i = 0; i < output_size; i++)
		{
			biases.output[i] = rand_float(min, max);
		}
	}
